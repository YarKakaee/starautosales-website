...
export async function uploadImage(file, path) {
	...
	// Since we convert all images to JPEG on client side, we should primarily expect JPEG
	...
	const hasValidExtension = fileName && validExtensions.some(ext => fileName.endsWith(ext));
	
	// If file has no type but has size and proper extension, allow it
	// Mobile browsers sometimes don't set MIME type correctly
	if (!hasValidMimeType && !hasValidExtension) {
		if (!file.type && file.size > 0) {
			...
			console.warn(`File type validation: type=${file.type}, name=${file.name}, size=${file.size}`);
			// Still allow if it's an image-like file (has size > 0)
			if (file.size > 0) {
				console.warn('Allowing upload despite type mismatch - will be handled by conversion');
			} else {
				throw new Error(
